...
const Navbar: React.FC<NavbarProps> = ({ animate }) => {
  ...
  return (
    <motion.div
      className="w-full mt-4 md:mt-8 flex flex-row items-center justify-between py-4 px-4 text-white"
      initial={{ opacity: 0, y: -20 }}
      animate={{ opacity: isAnimated ? 1 : 0, y: isAnimated ? 0 : -20 }}
      transition={{ duration: 0.8 }}
    >
      <motion.div
        className="flex flex-shrink-0 items-center gap-4 cursor-pointer"
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
        onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
      >
        <div className="relative p-2 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 overflow-hidden group">
          <img
      ...
      <div className="flex items-center justify-center gap-6 md:gap-10">
        {[
          ...
          >
            <svg viewBox={link.viewBox || "0 0 512 512"} className="w-6 h-6 md:w-8 md:h-8" fill="currentColor">
